...
function run_pep8 {
  ...
  DJANGO_SETTINGS_MODULE=zun_ui.test.settings ${command_wrapper} flake8
}
...
function run_pep8_changed {
    ...
    warn_on_flake8_without_venv
    diff -u --from-file /dev/null ${files} | DJANGO_SETTINGS_MODULE=zun_ui.test.settings ${command_wrapper} flake8 --diff
    exit
}

function run_sphinx {
    echo "Building sphinx..."
    DJANGO_SETTINGS_MODULE=zun_ui.test.settings ${command_wrapper} python setup.py build_sphinx
    echo "Build complete."
}
...
function run_tests {
  ...
  if [ $with_selenium -eq 0 -a $integration -eq 0 ]; then
      testopts="$testopts --exclude-dir=zun_ui/test/integration_tests"
  fi
...
function run_tests_all {
  echo "Running zun UI tests"
  export NOSE_XUNIT_FILE=zun_ui/nosetests.xml
  if [ "$NOSE_WITH_HTML_OUTPUT" = '1' ]; then
    export NOSE_HTML_OUT_FILE='dashboard_nose_results.html'
  fi
  ${command_wrapper} ${coverage_run} $root/manage.py test zun_ui --settings=openstack_dashboard.test.settings $testopts
...
function run_makemessages {
  ...
  echo -n "zun ui javascript: "
  babel_extract djangojs
  zun_JS_RESULT=$?

  cd ..
  if [ $check_only -eq 1 ]; then
    rm zun_ui/locale/django*.pot
  fi

  exit $(($zun_PY_RESULT || $zun_JS_RESULT))
}
